...
| `ZCS_SFDC_TOKEN` | Communication Scenario outbound · **Basic** (user = client id · pw = secret ใน `SFDC_DEV`) | `src/zcs_sfdc_token.sco1.xml` | | 🟦 Basic · published |
| Communication Arrangement `ZCA_SFDC_TOKEN` | Fiori config × `SFDC_DEV` — ไม่ขึ้น git | — | | ✅ Check Connection ✓ 2026-09-21 |
| `ZCL_UTILITY` | Class — `get_sfdc_token( )` · `parse_sfdc_token_response( )` (pure) | `src/zcl_utility.clas.abap` | ZARE002 · ZARI002 | 🟦 activate แล้ว · token จริง 200 (2026-09-21) |
| `ZCL_UTILITY` testclasses | parse token / error response — ไม่ต่อ SFDC | `src/zcl_utility.clas.testclasses.abap` | | 🟦 4 test เขียว |
